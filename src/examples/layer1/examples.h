...
#include "L-System\lsystem_app.h"
...
namespace octet {
  static app *app_factory(const char *name, int argc, char **argv) {
	...
	else if (!strcmp(name, "L-System")) return new lsystem_app(argc, argv); //  L-System application
  
	//else if (!strcmp(name, "my")) return new my_app(argc, argv); //  physics sample: continue here to learn how to annimate using physics
  }
  ...
  inline void run_examples(int argc, char **argv) {
    ...
    if (argc == 1) {
      ...
      app *myapp = app_factory("L-System", argc, argv);
