...
#include "math\vec4.h";
#include "platform\glut_specific.h"
#include "platform\windows_specific.h"



////////////////////////////////////////////////////////////////////////////////
//
// My modifications to make breakout game
//
//
...
namespace octet {
  ...
  class my_app : public octet::app {
    ...
    void simulate() {
      ...
      if (state == state_serving_left) {
		...
		// check collision with the breakoutBoxes
		for(int i = 0; i < 28; i++)
		{
			if (ball.collides_with(breakoutBoxes[i])) {
				ball_velocity_y = -ball_velocity_y;
			}
		}

		// check collision with the court right and left
        if (ball.collides_with(court[3])) {
          ball_velocity_x = -ball_velocity_x;
        } else if ( ball.collides_with(court[2])) {
          ball_velocity_x = -ball_velocity_x;
        }

        // check collision with the court end zones
        if (ball.collides_with(court[0])) {
          scores[0]--;
          state = scores[0] <= 3 ? state_game_over : state_serving_left;
        } 
      }
  ...
  public:
    ...
    my_app(int argc, char **argv) : app(argc, argv) {
    }
    ...
    void app_init() {
      ...
      ball.init(vec4(1, 0, 1, 1), 0, 0, 0.15f, 0.15f);
      bat[0].init(vec4(1, 1, 0, 1), 0, -3.7f, 1.0f, 0.2f);
	  ...
	  float y = 3;
		  for (int i = 0; i < 4; i++) 
		  {
			  x = -3.5f;
			  for(int j = 0; j < 7; j++){
				breakoutBoxes[j + i*7].init(vec4(1, 0, 0, 1), x, y, 1.0f, 0.25f);	
				x+=1.15f;
			  }
			  y-=0.4f;
		  }
    ...
    void draw_world(int x, int y, int w, int h) {
	  ...
	  // draw the breakoutBoxes
      for (int i = 0; i != 28; ++i) {
        breakoutBoxes[i].render(color_shader_, cameraToWorld);
      }
